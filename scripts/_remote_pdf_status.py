...
CMDS = [
("1. processed_pdfs.json (tail)", "cat /root/MINA_v2/signal_bot/processed_pdfs.json 2>/dev/null | python3 -m json.tool | tail -20"),
("2. ht_pdf_basari_orani son 15", r"""cd /root/MINA_v2 && python3 -c "
import sqlite3
...
rows = conn.execute('SELECT * FROM ht_pdf_basari_orani ORDER BY created_at DESC LIMIT 15').fetchall()
for r in rows:
    for c,v in zip(cols,r): print(f'{c}: {v}')
    print('---')
conn.close()
" """),
("3. Binance", r"""cd /root/MINA_v2 && source venv/bin/activate && python3 -c "
